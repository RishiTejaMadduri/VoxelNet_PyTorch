...
from dataloader.kitti import KITTI as Dataset
...
def run():
    ...
    val_dataset = Dataset(os.path.join(cfg.DATA_DIR, 'testing'), shuffle = False, aug = False, is_testset = False)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for (i, data) in enumerate(train_dataloader):
            ...
            if counter % args.print_freq == 0:
                # Print training info
                info = 'Train: {} @ epoch:{}/{} loss: {:.4f} reg_loss: {:.4f} cls_loss: {:.4f} cls_pos_loss: {:.4f} '                        'cls_neg_loss: {:.4f} forward time: {:.4f} batch time: {:.4f}'.format(
